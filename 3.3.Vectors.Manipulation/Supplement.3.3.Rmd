...
Similarly, if we want to round all of these numbers to 2 digits after the decimal place, we can apply the round function to the entire vector.
...
- Functions that are applied to each vector element (above)
- Functions that provide information about or summary statistics for a vector as a whole

One example of this is the `length()` function, which simply provides the length of a vector (the number of elements).
...
The function returns a named vector of length 6 (no matter the length of your vector argument) with several key statistics including the 1st and 3rd quartile. Quartiles divide the data into 4 quarters, the 1st quartile is the value at which 25% of the data is less than that value. The 3rd quartile is the value at which 75% of the data is less than that value.

For logical vectors, the functions `any()` and `all()` can be very useful. Both functions take a logical vector as an argument.

- `any()` returns `TRUE` if 1 or more element in the vector is `TRUE`
...
We can add, subtract, mutiply, divide, etc. each value in a vector by the same number. Here we multiply each value in the vector by 2.
...
We can also perform mathematical operations with 2 or more vectors containing multiple elements. For example, we can add the sequences `2:4` and `10:12`. Each of these vectors has a length of 3. 
...
We can also perform mathematical operations with 2 or more vectors of different lengths. The vector `2:7` has length 6, and the vector `c(2, 1, 5)` has length 3.
...
In the earlier examples when we used mathematical operators with a multiple element vector and a single value vector, the same thing was occurring. The single value is simply recycled, and therefore the same value is used for the mathematical operation with all elements in the longer vector.
...
These two vectors are "paired", there is coupling, or correspondence, between the two vectors at each index.
...
A Pearson correlation of 1 shows perfect correlation, so we can see that the data is very well correlated.
...
Below is fabricated data based on the data from Hahn-Hagerdal et al. These data represent the growth rates for 5 replicates for yeast grown on YP with glucose, and the growth rates for 5 replicates for yeast grown on YP with xylose.
...
As the p-value is less than 0.05, thus the growth measurements are significantly different between the two vectors.
